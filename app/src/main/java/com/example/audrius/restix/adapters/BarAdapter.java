...
public class BarAdapter extends RecyclerView.Adapter<RestourantViewHolder> {
    private ArrayList<Restourant> restourants;


    public BarAdapter(ArrayList<Restourant> restourants) {
    ...
    public void onBindViewHolder(final RestourantViewHolder holder, int position) {
        ...
        final Runnable r = new Runnable() {
            public void run() {
                ...
                if(restourant.getCat() == "bar" && newPosition >= 0 && newPosition <restourants.size()){
                    holder.UpdateCat(restourant, true);
                }
                else if(restourant.getCat()!="bar" && newPosition >= 0 && newPosition <restourants.size()){
